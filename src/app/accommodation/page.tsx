...
export default function AccommodationPage() {
  ...
  const saveTripState = async (updatedState: TripState) => {
    setSaving(true)
  ...
  const filteredSettlements = guestSettlements.filter(m => {
    ...
    if (activeTab === 'saturday') {
      // Find who did NOT stay on Saturday (sat === false) but paid for Saturday (or paid >= 750, indicating 2 days)
      // or has a refund due.
      return !m.sat && (m.paid > m.expected || m.paid >= 750)
    }
    if (activeTab === 'sunday') {
      // Find who stayed on Sunday
      return m.sun
    }
    return true
  })
  ...
  return (
    <div className="min-h-screen bg-black text-white px-6 pt-32 pb-24 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto z-10 relative">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-white/10 pb-6">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2 py-0.5 rounded bg-cyber-blue/10 text-cyber-blue text-[8px] font-mono tracking-widest uppercase font-black">
                Trip settlement tool v3.0
              </span>
            </div>
            <h1 className="text-3xl md:text-5xl font-black tracking-tight">
              ROOM ALLOCATOR & LEDGER
            </h1>
        ...
        {/* Conditional Header Alert for Saturday / Sunday */}
        {activeTab === 'saturday' && (
          <div className="mb-6 p-4 rounded-xl border border-yellow-500/20 bg-yellow-950/10 text-yellow-400 text-xs font-mono flex flex-col gap-1.5">
            <div className="flex items-center gap-2 font-bold uppercase">
              <AlertCircle size={14} />
              Saturday Refund Finder Mode
            </div>
            <p className="text-white/60">
              Listing guests who did <strong>NOT</strong> stay on Saturday (Day 2) but paid ₹750 or more (meaning they paid for Saturday), or have an overall positive payment balance.
              Refund them <strong>₹375</strong> (or their excess balance) for the second day room deduction.
            </p>
          </div>
        )}

        {activeTab === 'sunday' && (
          <div className="mb-6 p-4 rounded-xl border border-cyber-blue/20 bg-cyber-blue/5 text-cyber-blue text-xs font-mono flex flex-col gap-1.5">
            <div className="flex items-center gap-2 font-bold uppercase">
              <Users size={14} />
              Sunday Guest Audit Mode
            </div>
            <p className="text-white/60">
              Listing all guests who checked in on <strong>Sunday (Day 3)</strong>. We took <strong>{tripState.sun_rooms} rooms</strong> on Sunday, which holds a maximum capacity of <strong>{sunCapacity} guests</strong>. Currently checked-in on Sunday: <strong>{sunGuests} guests</strong>.
            </p>
          </div>
        )}
        ...
        <div className="p-8 rounded-[32px] bg-white/5 border border-white/10 backdrop-blur-xl">
          ...
          {/* Quick Info/Help Footer inside spreadsheet */}
          <div className="mt-6 border-t border-white/5 pt-4 text-[10px] font-mono text-white/40 flex justify-between items-center">
            <div>
              * Click checkboxes to select stay presence for each day.
            </div>
            <div>
              Total showing: {filteredSettlements.length} of {tripState.members.length} guests
            </div>
          </div>
        </div>
